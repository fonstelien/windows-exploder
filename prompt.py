...
class PromptEditor(Editor):
    ...
    def _evaluate(self):
        def cwd_content():
            subproc = subprocess.run("ls -lp | grep -v /$ ; tree -L 2 -dD",
        ...
        # List cwd contents
        if not args:
            self.program_status.set_result(self.mode_id, self.edit_text, 'success',
        ...
        if args[0] == 'mode':
            if len(args) > 1:
                self.change_mode = args[1] # Rest is handled by PromptWidgetHandler
            else:
                self.program_status.set_result(self.mode_id, self.edit_text, 'failure',
        ...
        try:
            subproc = subprocess.run(
                self.edit_text, shell=True, capture_output=True, encoding='UTF-8')
            ...
            self.program_status.set_result(self.mode_id, self.edit_text, 'error',
...
class PromptWidgetHandler(urwid.WidgetPlaceholder):
    ...
    def __init__(self, program_status):
        self.program_status = program_status
        self.widgets = dict()
        super(PromptWidgetHandler, self).__init__(self._init_mode(program_status.mode_id))
        ...
        if key == 'enter':
            ...
            if self.original_widget.change_mode:
                ...
                if next_mode_id in self.widgets.keys():
                    ...
                    self.program_status.set_result(curr_mode_id, command, 'success')
                ...
                # Initialize mode
                    ...
                    self.program_status.set_result(curr_mode_id, command, 'failure',
